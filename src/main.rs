...
fn load_or_create_config<P: AsRef<Path>>(config_path: P) -> Result<Config> {
    if config_path.as_ref().exists() {
        ...
        let rate_limit = if rate_limit > 0 { Some(rate_limit) } else { None };
...
fn validate_config(config: &Config) -> Result<()> {
    for log_file in &config.log_files {
        ...
        if let Err(e) = fs::metadata(path) {
            return Err(anyhow::anyhow!("Cannot access log file {}: {}", log_file, e));
        }
    }
    
    if !config.endpoint.starts_with("http://") && !config.endpoint.starts_with("https://") {
        return Err(anyhow::anyhow!("Endpoint URL must start with http:// or https://"));
    }
    
    let url_regex = Regex::new(r"^https?://[^\s/$.?#].[^\s]*$").unwrap();
    if !url_regex.is_match(&config.endpoint) {
        return Err(anyhow::anyhow!("Invalid endpoint URL format: {}", config.endpoint));
    }
...
#[serde(untagged)]
enum AttributeValue {
    StringValue { #[serde(rename = "stringValue")] value: String },
}

fn build_otlp_payload(line: &str, file: &str, severity_text: &str, severity_number: u8) -> OtlpLogRecord {
    OtlpLogRecord {
        resourceLogs: vec![
            ResourceLog {
                resource: Resource {
                    attributes: vec![
                        KeyValue {
                            ...
                            value: AttributeValue::StringValue { value: "rust-signoz-agent".into() },
                        },
                        KeyValue {
                            key: "host.name".into(),
                    ...
                    ScopeLog {
                        logRecords: vec![
                            LogRecord {
                                ...
                                body: LogBody { string_value: line.into() },
                                attributes: vec![
                                    KeyValue {
                                        key: "log.file".into(),
...
fn send_to_signoz(client: &reqwest::blocking::Client, endpoint: &str, log_line: &str, file: &str) {
    const MAX_RETRIES: usize = 3;
    let (severity_text, severity_number) = detect_severity_generic(log_line);
    let payload = build_otlp_payload(log_line, file, severity_text, severity_number);
    
    for attempt in 1..=MAX_RETRIES {
        match client.post(endpoint).json(&payload).send() {
            Ok(r) if r.status().is_success() => {
                println!("Successfully sent to SigNoz: [{}] ({}/{})", 
                         log_line, severity_text, severity_number);
                return;
            }
            Ok(r) => {
                eprintln!("Failed to send log to SigNoz: HTTP {} (attempt {}/{})", 
                          r.status(), attempt, MAX_RETRIES);
            }
            Err(e) => {
                eprintln!("HTTP error sending log to SigNoz: {} (attempt {}/{})", 
    ...
    eprintln!("Failed to send log after {} attempts, discarding: {}", MAX_RETRIES, log_line);
}

fn detect_severity_generic(line: &str) -> (&'static str, u8) {
    let regex = Regex::new(r"(?i)\b(INFO|ERROR|WARN|WARNING|DEBUG|CRITICAL|FATAL|NOTICE|TRACE)\b").unwrap();
...
fn main() -> Result<()> {
    ...
    let _sender_thread = thread::spawn(move || {
        ...
        while let Ok(entry) = rx.recv() {
            ...
            send_to_signoz(&client, &entry.endpoint, &entry.line, &entry.file);
        }
    });
